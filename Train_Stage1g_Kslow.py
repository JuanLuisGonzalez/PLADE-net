...
# -----------------------------------------------------------------------------
parser.add_argument('-mm', '--m_model', metavar='Mono Model', default='FAL_net2B_gep_mi', choices=model_names)
...
#                     default='Kitti_stage1g\\10-19-01_32\\FAL_net2B_g,e50es,b4,lr0.0001\\checkpoint.pth.tar',
...
def main():
    ...
    val_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.tbatch_size,
                                             num_workers=args.workers,
...
def train(train_loader, m_model, g_optimizer, epoch):
    ...
    for i, input_data0 in enumerate(train_loader):
        ...
        flip_in_grid = F.grid_sample(in_grid, flip_grid, align_corners=True)
        flip_in_grid[:, 0, :, :] = -flip_in_grid[:, 0, :, :]
        ...
        pan, disp = m_model(torch.cat((left_view, F.grid_sample(right_view, flip_grid, align_corners=True)), 0),
                            torch.cat((in_grid, flip_in_grid), 0),
