...
def FAL_net2B_gep2s(data=None, no_levels=49):
...
class BackBone(nn.Module):
   def __init__(self, batchNorm=True, no_in=3, no_flow=1, no_out=64, no_ep=8):
       ...
       self.conv2 = conv_elu(self.batchNorm, 128 + 64 + no_ep + no_flow, 256, pad=1, stride=2)
       ...
       self.conv6 = conv_elu(self.batchNorm, 256 * 2 + no_ep + no_flow, 256, pad=1, stride=2)
   ...
   def forward(self, x, y, grid, flow):
       _, _, H, W = x.shape
       ...
       out_conv1 = self.conv1_1(self.conv1(torch.cat((out_conv0, self.conv_ep1(grid), flow), 1)))
       ...
       dflow = F.interpolate(flow, size=(out_conv1.shape[2], out_conv1.shape[3]), align_corners=True, mode='bilinear')
       out_conv2 = self.conv2_1(self.conv2(torch.cat((out_conv1, out_conv0lr, self.conv_ep2(dgrid), dflow), 1)))

       dgrid = F.interpolate(grid, size=(out_conv2.shape[2], out_conv2.shape[3]), align_corners=True, mode='bilinear')
       dflow = F.interpolate(flow, size=(out_conv2.shape[2], out_conv2.shape[3]), align_corners=True, mode='bilinear')
       out_conv3 = self.conv3_1(self.conv3(torch.cat((out_conv2, self.conv_ep3(dgrid), dflow), 1)))

       dgrid = F.interpolate(grid, size=(out_conv3.shape[2], out_conv3.shape[3]), align_corners=True, mode='bilinear')
       dflow = F.interpolate(flow, size=(out_conv3.shape[2], out_conv3.shape[3]), align_corners=True, mode='bilinear')
       out_conv4 = self.conv4_1(self.conv4(torch.cat((out_conv3, self.conv_ep4(dgrid), dflow), 1)))

       dgrid = F.interpolate(grid, size=(out_conv4.shape[2], out_conv4.shape[3]), align_corners=True, mode='bilinear')
       dflow = F.interpolate(flow, size=(out_conv4.shape[2], out_conv4.shape[3]), align_corners=True, mode='bilinear')
       out_conv5 = self.conv5_1(self.conv5(torch.cat((out_conv4, self.conv_ep5(dgrid), dflow), 1)))
       ...
       # One strided conv encoder stage
       out_conv1r = self.conv1_1(self.conv1(torch.cat((out_conv0r, self.conv_ep1(grid), flow), 1)))
       ...
       dflow = F.interpolate(flow, size=(out_conv1r.shape[2], out_conv1r.shape[3]), align_corners=True, mode='bilinear')
       out_conv2r = self.conv2_1(self.conv2(torch.cat((out_conv1r, out_conv0lr, self.conv_ep2(dgrid), dflow), 1)))

       dgrid = F.interpolate(grid, size=(out_conv2r.shape[2], out_conv2r.shape[3]), align_corners=True, mode='bilinear')
       dflow = F.interpolate(flow, size=(out_conv2r.shape[2], out_conv2r.shape[3]), align_corners=True, mode='bilinear')
       out_conv3r = self.conv3_1(self.conv3(torch.cat((out_conv2r, self.conv_ep3(dgrid), dflow), 1)))

       dgrid = F.interpolate(grid, size=(out_conv3r.shape[2], out_conv3r.shape[3]), align_corners=True, mode='bilinear')
       dflow = F.interpolate(flow, size=(out_conv3r.shape[2], out_conv3r.shape[3]), align_corners=True, mode='bilinear')
       out_conv4r = self.conv4_1(self.conv4(torch.cat((out_conv3r, self.conv_ep4(dgrid), dflow), 1)))

       dgrid = F.interpolate(grid, size=(out_conv4r.shape[2], out_conv4r.shape[3]), align_corners=True, mode='bilinear')
       dflow = F.interpolate(flow, size=(out_conv4r.shape[2], out_conv4r.shape[3]), align_corners=True, mode='bilinear')
       out_conv5r = self.conv5_1(self.conv5(torch.cat((out_conv4r, self.conv_ep5(dgrid), dflow), 1)))

       dgrid = F.interpolate(grid, size=(out_conv5.shape[2], out_conv5.shape[3]), align_corners=True, mode='bilinear')
       dflow = F.interpolate(flow, size=(out_conv5.shape[2], out_conv5.shape[3]), align_corners=True, mode='bilinear')
       out_conv6 = self.conv6_1(self.conv6(torch.cat((out_conv5, out_conv5r, self.conv_ep6(dgrid), dflow), 1)))
...
class FAL_net(nn.Module):
   def __init__(self, batchNorm, no_levels):
       ...
       self.no_fac = 1
       self.backbone = BackBone(batchNorm, no_in=3, no_flow=2, no_out=self.no_levels)
       self.softmax = nn.Softmax(dim=1)
       self.elu = nn.ELU(inplace=True)
       self.sigmoid = nn.Sigmoid()

       # An additional 1x1 conv layer on the logits (not shown in paper). Its contribution should not be much.
       self.conv0 = nn.Conv2d(self.no_levels, self.no_fac * self.no_levels, kernel_size=1, stride=1, padding=0,
                              bias=True)
   ...
   def forward(self, input_left, input_right, in_grid, min_disp, max_disp,
       ...
       flow = torch.ones(B, 2, H, W).type(input_left.type())
       flow[:, 0, :, :] = min_disp * flow[:, 0, :, :] / 100  # normalized?
       flow[:, 1, :, :] = max_disp * flow[:, 1, :, :] / 100  # normalized?

       # Synthesize zoomed image
       dlog = self.backbone(input_left, input_right, in_grid, flow)
       ...
       for n in range(0, self.no_levels * self.no_fac):
           ...
           if n == 0:
               ...
               Dprob = torch.cat((Dprob, F.grid_sample(dlog0[:, n, :, :].unsqueeze(1), out_grid, align_corners=True)),
                                 1)
