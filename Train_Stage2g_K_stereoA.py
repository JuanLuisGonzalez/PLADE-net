...
parser.add_argument('--fix_model', dest='fix_model',
                    default='Kitti_stage2g/11-14-13_44/FAL_net2B_gep,e20es7800,b4,lr5e-05/checkpoint.pth.tar')
parser.add_argument('--pretrained', dest='pretrained',
                    default='Kitti_stage1g/11-12-20_47/FAL_net2B_gepss,e52es,b4,lr0.0001/checkpoint9533.pth.tar',
...
def train(train_loader, m_model, fix_model, g_optimizer, epoch, deb_path):
    ...
    for i, input_data0 in enumerate(train_loader):
        ...
        ### Run forward pass ###
        min_disp = max_disp * args.min_disp / args.max_disp
        pan, disp, mask0, mask1 = m_model(torch.cat((left_view, right_view), 0),
        ...
        # # lrdisp = F.grid_sample(lrdisp, flip_grid, align_corners=True)
        ...
        if args.a_sm > 0:
            ...
            sm_loss = (smoothness(left_view, ldisp, gamma=2) +
                       smoothness(right_view, rdisp, gamma=2)) / 2
        ...
        if args.a_mr > 0:
            ...
            mirror_loss = (torch.mean(nmaxl * (1 - O_L) * torch.abs(ldisp - mldisp)) +
                           torch.mean(nmaxr * (1 - O_R) * torch.abs(rdisp - mrdisp))) / 2
        ...
        if args.a_dcl > 0:
            ...
            deep_corr_loss = (corrL1Loss(vgg(y_dl), vgg_left, layer=args.dcl_layer) +
                              corrL1Loss(vgg(y_dr), vgg_right, layer=args.dcl_layer)) / 2
        ...
        if args.a_dcml > 0:
            # Normalize estimated depths and matted depths as images
        ...
        if args.a_ddm > 0:
            with torch.no_grad():
                if args.use_wmean_fac:
                    ksize = 5
                    llmean = F.avg_pool2d((ldisp.detach()+mldisp)/2, kernel_size=ksize, padding=(ksize - 1)//2, stride=ksize)
                    ...
                    rlmean = F.avg_pool2d((rdisp.detach()+mrdisp)/2, kernel_size=ksize, padding=(ksize - 1)//2, stride=ksize)
                ...
                ddm_lmask0 = torch.sum(torch.abs(left_view - F.grid_sample(right_view, warp_grid0, align_corners=True)),
                                       dim=1, keepdim=True) < \
                             (1 - thres) * torch.sum(torch.abs(
                    left_view - F.grid_sample(right_view, warp_grid1, align_corners=True)),dim=1, keepdim=True)
                ddm_lmask1 = torch.sum(torch.abs(left_dm - F.grid_sample(right_dm, warp_grid0, align_corners=True)),
                                       dim=1, keepdim=True) < \
                             (1 - thres) * torch.sum(torch.abs(ldisp.detach() -
                ...
                ddm_rmask0 = torch.sum(torch.abs(right_view - F.grid_sample(left_view, warp_grid0, align_corners=True)),
                                       dim=1, keepdim=True) < \
                             (1 - thres) * torch.sum(torch.abs(
                    right_view - F.grid_sample(left_view, warp_grid1, align_corners=True)), dim=1, keepdim=True)
                ddm_rmask1 = torch.sum(torch.abs(right_dm - F.grid_sample(left_dm, warp_grid0, align_corners=True)),
                                       dim=1, keepdim=True) < \
                             (1 - thres) * torch.sum(torch.abs(rdisp.detach() -
