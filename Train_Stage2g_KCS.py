...
parser.add_argument('-n1', '--dataName1', metavar='Data Set Name 1', default='Cityscapes_jpg', choices=dataset_names)
parser.add_argument('-relbase1', '--rel_base1', default=22 / 54, help='Relative baseline of training dataset')
...
# ---------------------------------------------------------------------------------------------------------------------
parser.add_argument('-mm', '--m_model', metavar='Mono Model', default='PLADE_net_gep', choices=model_names)
...
parser.add_argument('-lrc_loss', '--a_lrc', default=0.0, help='LR const loss')
# ---------------------------------------------------------------------------------------------------------------------
...
parser.add_argument('--milestones', default=[4, 9], metavar='N', nargs='*',
...
parser.add_argument('--fix_model', dest='fix_model',
                   default='Kitti_stage1g/11-28-01_30/PLADE_net_gep,e52es7400,b8,lr0.0001/checkpoint.pth.tar')
...
def main():
   ...
   co_transform_c = data_gtransforms.Compose([
       data_gtransforms.RandomResizeCrop((args.crop_height, args.crop_width), down=0.65, up=1.1),
       data_gtransforms.RandomHorizontalFlipG(),
       data_gtransforms.RandomGamma(min=0.8, max=1.2),
       data_gtransforms.RandomBrightness(min=0.5, max=2.0),
       data_gtransforms.RandomCBrightness2(min=0.8, max=1.0),
   ...
   input_path = os.path.join(args.data, args.dataName1)
   [train_dataset1, _] = Datasets.__dict__[args.dataName1](split=1,
                                                           root=input_path,
                                                           transform=input_transform,
                                                           target_transform=target_transform,
                                                           co_transform=co_transform_c,
                                                           max_pix=args.max_disp * args.rel_base1,
                                                           fix=True,
                                                           use_grid=True,
                                                           read_matted_depth=args.a_dcml > 0 or args.a_ddm > 0)
   ...
   # Torch Data Loaders
   train_loader0 = torch.utils.data.DataLoader(train_dataset0, batch_size=args.batch_size // 2,
                                               num_workers=args.workers,
                                               pin_memory=False, shuffle=True)
   train_loader1 = torch.utils.data.DataLoader(train_dataset1, batch_size=args.batch_size // 2,
                                               num_workers=args.workers,
                                               pin_memory=False, shuffle=True)
   ...
   for epoch in range(args.start_epoch, args.epochs):
       ...
       # train for one epoch
       train_loss = train(train_loader0, train_loader1, m_model, fix_model, g_optimizer, epoch, deb_path)
...
def train(train_loader0, train_loader1, m_model, fix_model, g_optimizer, epoch, deb_path):
   global args
   train_loader = zip(train_loader0, train_loader1)
   ...
   for i, (input_data0, input_data1) in enumerate(train_loader):
       ...
       # Read training data from dataset1 and combine in a single batch
       left_view = torch.cat((left_view, input_data1[0][0].cuda()), 0)
       right_view = torch.cat((right_view, input_data1[0][1].cuda()), 0)
       in_grid = torch.cat((in_grid, input_data1[1][0].cuda()), 0)
       max_disp = torch.cat((max_disp, input_data1[2].unsqueeze(1).unsqueeze(1).type(left_view.type())), 0)
       if args.a_dcml > 0 or args.a_ddm > 0:
           left_dm = torch.cat((left_dm, input_data1[3][0].cuda()), 0)
           right_dm = torch.cat((right_dm, input_data1[3][1].cuda()), 0)
       ...
       if args.a_dcl > 0:
           ...
           vgg_left = vgg(left_view[:, :, :, int(0.20 * W)::])
           vgg_right = vgg(right_view[:, :, :, 0:int(0.80 * W)])
           ...
           deep_corr_loss = (corrL1Loss(vgg(y_dl[:, :, :, int(0.20 * W)::]), vgg_left, layer=args.dcl_layer) +
                             corrL1Loss(vgg(y_dr[:, :, :, 0:int(0.80 * W)]), vgg_right, layer=args.dcl_layer)) / 2

       # Compute lr loss
       lrc_loss = 0
       # if args.a_lrc > 0:
       #     lrc_loss = (torch.mean(nmaxl * O_L[:, :, :, int(0.20 * W)::] *
       #                            torch.abs(ldisp.detach() - rdispl)[:, :, :, int(0.20 * W)::]) +
       #                    torch.mean(nmaxr * O_R[:, :, :, 0:int(0.80 * W)] *
       #                               torch.abs(rdisp.detach() - ldispr)[:, :, :, 0:int(0.80 * W)])) / 2
       ...
       if args.a_dcml > 0:
           ...
           deep_corr_matting_loss = (corrL1Loss(vgg(y_dl[:, :, :, int(0.20 * W)::]),
                                                vgg(y_dml[:, :, :, int(0.20 * W)::]), layer=args.dcml_feat) +
                                     corrL1Loss(vgg(y_dr[:, :, :, 0:int(0.80 * W)]),
                                                vgg(y_dmr[:, :, :, 0:int(0.80 * W)]), layer=args.dcml_feat)) / 2
       ...
       if args.a_ddm > 0:
           with torch.no_grad():
               ...
               if args.use_wmean_fac:
                   ksize = args.ksize
                   avg_disp = ldisp.detach() # (ldisp.detach() + mldisp) / 2
                   llmean = F.avg_pool2d(avg_disp, kernel_size=ksize, padding=(ksize - 1) // 2, stride=ksize)
                   ...
                   avg_disp = rdisp.detach() #(rdisp.detach() + mrdisp) / 2
                   rlmean = F.avg_pool2d(avg_disp, kernel_size=ksize, padding=(ksize - 1) // 2, stride=ksize)
